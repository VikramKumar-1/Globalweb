...
import { getSubdomainHomepageFaqs, saveSubdomainHomepageFaqs } from '../actions';
...
export default function FaqsSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    const res = await saveSubdomainHomepageFaqs(cleanedFaqs);
    setSaving(false);
    
    if (res.success) {
      setFaqs(cleanedFaqs);
      showToast("Market Area FAQs saved successfully!");
    } else {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-black text-gray-900 font-poppins flex items-center gap-2">
            <HelpCircle className="text-[#1a8b4c]" /> Market Area FAQ Accordions
          </h1>
          <p className="text-sm text-gray-500 mt-1">Configure the frequently asked questions segment visible at the bottom of the market area subdomains. Supports <strong>{'{location}'}</strong>.</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
