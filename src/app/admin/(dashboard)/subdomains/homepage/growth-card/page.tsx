...
import { getSubdomainHomepageAboutCard, saveSubdomainHomepageAboutCard } from '../actions';
...
export default function GrowthCardSettingsPage() {
  ...
  const handleSave = async () => {
    setSaving(true);
    const res = await saveSubdomainHomepageAboutCard(data);
    setSaving(false);
    ...
    <div className="max-w-4xl mx-auto py-8 px-4 relative space-y-10">
      ...
      <div>
        <h1 className="text-2xl font-black text-gray-950 font-poppins flex items-center gap-2">
          <Layout className="text-[#1a8b4c]" /> Market Area Growth Card Settings
        </h1>
        <p className="text-sm text-gray-500 mt-1">Configure the content of the "Data-Driven Growth Agency" card on the market area subdomains. Supports <strong>{'{location}'}</strong>.</p>
      </div>
